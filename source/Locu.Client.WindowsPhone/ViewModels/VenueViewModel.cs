using Locu.Client.WindowsPhone.Resources;
...
namespace Locu.Client.WindowsPhone.ViewModels
{
    public class VenueViewModel : ViewModelBase
    {
        ...
        public static async Task<VenueViewModel> Create()
        {
            ...
            //If cached copy does not exists in isolated storage
            //try to load from an embedded resource
            if (string.IsNullOrEmpty(cachedData))
                cachedData = GetDataFromAppResource();
        ...
        private static string GetDataFromAppResource()
        {
            string data = "";

            try
            {
                var appResource = App.GetResourceStream(new Uri("Data/venue.json", UriKind.Relative));
                data = new StreamReader(appResource.Stream).ReadToEnd();
            }
            catch { }
        ...
        {
            var request = new VenueDetailsRequest(Keys.LocuApiKey, Keys.LocuVenuId);
